from app.utils.pure_db import execute, fetch
...
async def db_insert_personal(user):
    query = """insert into personal(name, password, mail, role)
               values(:name, :password, :mail, :role)"""
    values = dict(user)
    await execute(query, False, values)


async def db_check_personal(name, password):
    query = """select * from personal where name =:name and password=:password"""
